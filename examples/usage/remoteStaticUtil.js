 ...
 * 获取远程静态资源的 url
 * @param {*} options 选项
 * @param {boolean} options.random 是否随机选择一个 url
 * @param {number} options.index 远程静态资源访问地址引索，默认使用第一个地址
 * @returns {string} 远程静态资源的 url
 */
export function getRemoteStaticUrl(options) {
  // 开发环境下，强制使用 第一个地址
  if (isDevelopment) {
 ...
 * @param {string} filePath 视频路径,不需要添加前缀 `/`
 * @param {*} options 选项
 * @param {boolean} options.random 是否随机选择一个 url
 * @param {number} options.index 远程静态资源访问地址引索，默认使用第一个地址
 * @returns {string} 远程静态视频的 url
 */
export function getRemoteStaticVideoUrl(filePath, options) {
  return `${getRemoteStaticUrl(options)}/videos/${removePrefix(filePath)}`
}

/**
 * 获取远程静态音频效的 url 如果没有传入音频效路径，默认返回根目录
 * @param {string} filePath 音频效路径,不需要添加前缀 `/`
 * @param {*} options 选项
 * @param {boolean} options.random 是否随机选择一个 url
 * @param {number} options.index 远程静态资源访问地址引索，默认使用第一个地址
 * @returns {string} 远程静态音频效的 url
 */
export function getRemoteStaticSoundUrl(filePath, options) {
  return `${getRemoteStaticUrl(options)}/sounds/${removePrefix(filePath)}`
}

/**
 * 获取远程静态图片的 url 如果没有传入图片路径，默认返回根目录
 * @param {string} filePath 图片路径,不需要添加前缀 `/`
 * @param {*} options 选项
 * @param {boolean} options.random 是否随机选择一个 url
 * @param {number} options.index 远程静态资源访问地址引索，默认使用第一个地址
 * @returns {string} 远程静态图片的 url
 */
export function getRemoteStaticImageUrl(filePath, options) {
  return `${getRemoteStaticUrl(options)}/images/${removePrefix(filePath)}`
}

/**
 * 获取远程静态字体的 url 如果没有传入字体路径，默认返回根目录
 * @param {string} filePath 字体路径,不需要添加前缀 `/`
 * @param {*} options 选项
 * @param {boolean} options.random 是否随机选择一个 url
 * @param {number} options.index 远程静态资源访问地址引索，默认使用第一个地址
 * @returns {string} 远程静态字体的 url
 */
export function getRemoteStaticFontUrl(filePath, options) {
